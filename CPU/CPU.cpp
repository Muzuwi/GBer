...
#define SLA_R(a)	flagMask(Registers.a >> 7); Registers.a = Registers.a << 1; Registers.a &= ~(1); if(Registers.a == 0){ setZ; }
#define SRA_R(a)	flagMask(Registers.a & 1); Registers.a = (Registers.a >> 1) | (Registers.a & 0x80); if(Registers.a == 0){ setZ; }
#define SLA_D(a)	flagMask(RAM::read(a) >> 7); RAM::write(a, RAM::read(a) << 1); RAM::write(a, RAM::read(a) & ~(1)); if(RAM::read(a) == 0){ setZ; }
#define SRA_D(a)	flagMask(RAM::read(a) & 1); RAM::write(a, (RAM::read(a) >> 1) | (RAM::read(a) & 0x80)); if(RAM::read(a) == 0){ setZ; }

#define RL_R(a)		int temp = readC; \
					flagMask(Registers.a >> 7); \
					Registers.a = Registers.a << 1; \
					Registers.a = (Registers.a & (~1) | temp); \
					if(Registers.a == 0){ setZ; }

#define RR_R(a)		int temp = readC; \
					flagMask(Registers.a >> 7); \
					Reigsters.a = Registers.a >> 1; \
					Registers.a = (Registers.a & (~(1 << 7))) | (temp << 7); \
					if(Registers.a == 0){ setZ; }

#define RL_D(a)		int temp = readC; \
					flagMask(RAM::read(a) >> 7); \
					RAM::write(a, RAM::read(a) << 1 ); \
					RAM::write(a, RAM::read(a) & (~1) | temp); \
					if(RAM::read(a) == 0){ setZ; }

#define RR_D(a)		int temp = readC; \
					flagMask(RAM::read(a) >> 7); \
					RAM::write(a, RAM::read(a) >> 1); \
...
x(0x7,0x0F,1,4,flagMask(Registers.A & 1); Registers.A = (Registers.A >> 1) | ((Registers.A & 1) << 7)) \
x(0x0,0x10,2,4,				   ) \
...
x(0x7,0x17,1,4, int tempor = readC; flagMask(Registers.A >> 7); Registers.A = (Registers.A << 1) | tempor) \
...
x(0x0,0xE0,2,12,               ) \
x(0x0,0xE1,1,12,               ) \
x(0x0,0xE2,2,8,LD_D_R(Registers.C, A)    ) \
x(0x0,0xE5,1,16,               ) \
x(0x5,0xE6,2,8,                ) \
x(0x0,0xE7,1,16,               ) \
...
y(0x0, 0x0, 0x8D,2,8, resetBitN_D(1, L)               ) \
y(0x0, 0x0, 0x8E,2,15,resetBitN_R(1, HL_p)                ) \
...
y(0x0, 0x0, 0x94,2,8, resetBitN_D(2, H)               ) \
y(0x0, 0x0, 0x95,2,8, resetBitN_R(2, L)               ) \
y(0x0, 0x0, 0x96,2,15,resetBitN_R(2, HL_p)                ) \
...
y(0x0, 0x0, 0xAE,2,15,resetBitN_R(5, HL_p)                ) \
y(0x0, 0x0, 0xAF,2,8, resetBitN_R(5, A)               ) \
y(0x0, 0x0, 0xB0,2,8, resetBitN_D(6, B)               ) \
...
y(0x0, 0x0, 0xB6,2,15,resetBitN_R(6, HL_p)                ) \
y(0x0, 0x0, 0xB7,2,8, resetBitN_D(6, A)               ) \
	...
	*/
	bool cycle(){
		...
		if(prefix != 0xCB){
			...
			switch(opcode){
				#define y(setMask,unsetMask,op,byte,cycle,ins) case op:{ ins; Registers.F |= setMask; cycles += cycle; Registers.PC += byte; break; }
