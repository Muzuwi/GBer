...
#define ADD_R_D(a, b) if(CPU::Registers.a + b > 0xFF){ setC; }; \
					  CPU::Registers.a += b; \
					  if(CPU::Registers.a == 0){ setZ; }												//         !!!! IMPORTANT: DO HALF CARRY FLAG

#define ADC_R(a) if(CPU::Registers.A + CPU::Registers.a > 0xFF){ setC; }; \
				 CPU::Registers.A = CPU::Registers.A + CPU::Registers.a + readC; \
				 if(CPU::Registers.A == 0){ setZ; }														//         !!!! IMPORTANT: DO HALF CARRY FLAG

#define ADC_D(a) if(CPU::Registers.A + a > 0xFF){ setC; }; \
				 CPU::Registers.A = CPU::Registers.A + a + readC; \
				 if(CPU::Registers.A == 0){ setZ; }														//         !!!! IMPORTANT: DO HALF CARRY FLAG

#define INC_R(a) CPU::Registers.a += 1; if(CPU::Registers.a == 0){ setZ; }								//         !!!! IMPORTANT: DO HALF CARRY FLAG
#define INC_D(a) RAM::write(a, RAM::read(a) + 0x01); if(RAM::read(a) == 0){ setZ; }						//		   !!!! IMPORTANT: DO HALF CARRY FLAG

#define DEC_R(a) CPU::Registers.a -= 1; if(CPU::Registers.a == 0){ setZ; }								//         !!!! IMPORTANT: DO HALF CARRY FLAG
#define DEC_D(a) RAM::write(a, RAM::read(a) - 0x01); if(RAM::read(a) == 0){ setZ; }						//		   !!!! IMPORTANT: DO HALF CARRY FLAG


#define SUB_R_R(a,b) if(CPU::Registers.a < CPU::Registers.b){ setC; }; \
				     CPU::Registers.a -= CPU::Registers.b; \
				 	 if(CPU::Registers.a == 0){ setZ; }			//		   !!!! IMPORTANT: DO HALF CARRY FLAG (MIGHT ALSO BE TERRIBLY WRONG)

#define SUB_R_D(a,b) if(CPU::Registers.a < b){ setC; }; \
				 	 CPU::Registers.a -= b; \
				 	 if(CPU::Registers.a == 0){ setZ; }			//		   !!!! IMPORTANT: DO HALF CARRY FLAG (MIGHT ALSO BE TERRIBLY WRONG)



#define SBC_R(a) if(CPU::Registers.A < CPU::Registers.A - CPU::Registers.a - readC){ setC; } \
				 CPU::Registers.A = CPU::Registers.A - CPU::Registers.a - readC; \
				 if(CPU::Registers.A == 0){ setZ; }				//		   !!!! IMPORTANT: DO HALF CARRY FLAG (MIGHT ALSO BE TERRIBLY WRONG)

#define SBC_D(a) if(CPU::Registers.A < CPU::Registers.A - a - readC){ setC; } \
				 CPU::Registers.A = CPU::Registers.A - a - readC; \
				 if(CPU::Registers.A == 0){ setZ; }				//		   !!!! IMPORTANT: DO HALF CARRY FLAG (MIGHT ALSO BE TERRIBLY WRONG)

#define CP_R(a)	if(CPU::Registers.A - CPU::Registers.a == 0){ setZ; } \
				else if(CPU::Registers.A < CPU::Registers.a){ setC; } 							//			!!!! IMPORTANT: DO HALF CARRY FLAG

#define CP_D(a) if(CPU::Registers.A - a == 0){ setZ; } \
				else if(CPU::Registers.A < a){ setC; } 											//			!!!! IMPORTANT: DO HALF CARRY FLAG
						

#define ADD_HL_N(a)	if(readHL() + a > 0xFFFF){ setC; } ;\
...
#define OR_D(a) CPU::Registers.A |= a; 					if(CPU::Registers.A == 0){ setZ; }
...
x(0x0, 0x0, 0x09,1,8,ADD_HL_N(readBC())		) \
...
x(0x0, 0x0, 0x19,1,8,ADD_HL_N(readDE()) ) \
...
x(0x0, 0x0, 0x39,1,8,ADD_HL_N(CPU::Registers.PC)  ) \
...
x(0x0, 0x0, 0x86,1,8,ADD_R_D(A, HL_mv)) \
...
x(0x0, 0x0, 0x8E,1,8,ADC_D(HL_mv)     ) \
...
x(0x0, 0x0, 0x96,1,8,SUB_R_D(A, HL_mv)) \
...
x(0x0, 0x0, 0x9E,1,8,SBC_D(HL_mv)	   ) \
...
y(0x6, 0x0, 0x36,2,15,SWAP_D(HL_v)                ) \
...
y(0x6, 0x0, 0x3E,2,15,SWAP_D(HL_v)                ) \
	...
	*/
	bool cycle(){
		...
		if(prefix != 0xCB){
			...
			switch(opcode){
				#define y(unsetMask,setMask,op,byte,cycle,ins) case op:{ ins; Registers.F |= setMask; Registers.F &= ~unsetMask; cycles += cycle; Registers.PC += byte; break; }
				INSTRUCTIONS_CB(y)
				#undef y
