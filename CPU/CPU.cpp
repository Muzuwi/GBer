...
#define ADC_R(a) if(CPU::Registers.A + CPU::Registers.a > 0xFF){ setC; } else { unsetC; }; \
				 if( (CPU::Registers.A & 0x0F) + (CPU::Registers.a & 0x0F) > 0x0F){ setH; } else { unsetH; } \
				 CPU::Registers.A = CPU::Registers.A + CPU::Registers.a + readC; \
				 if(CPU::Registers.A == 0){ setZ; } else { unsetZ; }														

#define ADC_D(a) if(CPU::Registers.A + a > 0xFF){ setC; } else { unsetC; }; \
				 if( (CPU::Registers.A & 0x0F) + (a & 0x0F) > 0x0F){ setH; } else { unsetH; } \
...
#define SUB_R_R(a,b) if(CPU::Registers.a < CPU::Registers.b){ setC; } else { unsetC; }; \
				     ...
				     CPU::Registers.a -= CPU::Registers.b; \
				 	 if(CPU::Registers.a == 0){ setZ; } else { unsetZ; }			//		   MIGHT BE TERRIBLY WRONG
...
#define SBC_R(a) if(CPU::Registers.A < CPU::Registers.A - CPU::Registers.a - readC){ setC; } else { unsetC; } \
				 if( (CPU::Registers.A & 0x0F) - (CPU::Registers.a & 0x0F) < 0){ setH; } else { unsetH; } \
				 CPU::Registers.A = CPU::Registers.A - CPU::Registers.a - readC; \
				 if(CPU::Registers.A == 0){ setZ; } else { unsetZ; }				//		   MIGHT BE TERRIBLY WRONG

#define SBC_D(a) if(CPU::Registers.A < CPU::Registers.A - a - readC){ setC; } else { unsetC; } \
...
#define ADD_HL_N(a)	if(readHL() + a > 0xFFFF){ setC; } else { unsetC; } \
					if( (readHL() & 0x0F) + (a & 0x0F) > 0x0F){ setH; } else { unsetH; } \
...
x(0x4, 0x0, 0x34,1,12,writeHL(readHL() + 0x01) 							) \
x(0x0, 0x4, 0x35,1,12,writeHL(readHL() - 0x01) 							) \
x(0x0, 0x0, 0x36,2,12,writeHL(d8)	   									) \
x(0x6, 0x1, 0x37,1,4,setC           									) \
x(0x0, 0x0, 0x38,2,8,JR_CC_N(readC, r8)  								) \
x(0x4, 0x0, 0x39,1,8,ADD_HL_N(CPU::Registers.PC)  						) \
...
x(0xC, 0x0, 0xE8,2,16,ADD_R_D(SP, r8)									) \
